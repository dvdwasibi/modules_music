...
ProposalPublisherProxy _proposalPublisher = new ProposalPublisherProxy();
...
class ContextListenerImpl extends ContextListener {
    ...
    if (result.values.containsKey(_kMusicArtistTopic)) {
      ...
      if (data.containsKey('name')) {
        ...
        if (events != null && events.length > 0) {
          ...
          createProposal(events, data['name']);
        } else {
  ...
  /// search.
  void createProposal(List<Event> events, String artistName) {
      ...
      ..onSelected = <Action>[
        new Action()
          ..createStory = (new CreateStory()
            // TODO (dayang@): Update this once the Concert module is implemeted
